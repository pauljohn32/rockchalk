             ...
             extraFuns)
{
    ...
    harvest <- function(sl, name) {
        ...
        for(i in seq_along(sl)) {
            ...
            if (!is.null(y)) res[i] <- y else res[i] <- NULL
        }
        if (length(sl) -  sum(sapply(res, is.null)) > 0) nonNull <- TRUE
        final <- res
        names(final) <- names(sl)
        attr(final, "nonNull") <- TRUE
        final
    }
