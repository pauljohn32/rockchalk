...
\begin{abstract}
\noindent \code{rockchalk} version 1.8.120 marked the first effort
to introduce decimal-centering of columns in the \code{outreg} table-writer
for regression models. This has been the most widely requested feature
for a while. Years ago, I declined to do it because it seemed a needless
complication. However, in 2018 I was persuaded we should try harder
on decimal centering. In today's version of rockchalk, 1.8.136, I
have working examples of 2 centering methods, \code{dcolumn} and
\code{siunitx}. \code{dcolumn} was the first method attempted. It
is relatively widely used. It is simpler to work with inside the \code{outreg}
code, but the results are not quite as good as \code{siunitx}. \code{dcolumn}
...
massive framework. It required much more alteration of \code{rockchalk}
itself. There is much more danger of outright failure because of the
immensity of \code{siunitx} itself. However, there are hopeful signs
it can deal with some very tough examples. See Table \ref{tab:ex16s}
I have a solution using \code{siunitx} for a troublesome table that
failed badly with \code{dcolumn} (Table \ref{tab:ex16d}).

\noindent I am asking friends and colleagues to explore and test.
Run \code{outreg} as usual to have my traditional, simple solution
that has been good enough for 13 years for most people. Then insert
an additional \code{outreg} argument to ask for column centering.
Try \code{centering = \textquotedbl dcolumn\textquotedbl} or \code{centering = \textquotedbl siunitx\textquotedbl}.

This document might become a vignette in \code{rockchalk}, but right
now it is just a brief explanation and a giant collection of example
tables. It is a plea for insight \& advice. This essay is not in the
package, but it is in the repository: \url{https://github.com/pauljohn32/rockchalk}.
Anybody with a GitHub account can clone that. This vignette will be
included in \code{rockchalk} itself under the name ``outreg''.
\end{abstract}
\tableofcontents{}
...
The new arguments for column centering are \code{dcolumn} and \code{siunitx}.
The default is \code{FALSE}, meaning there will be no effect. If
FALSE, the \code{outreg} table will be the same as it always was.
However, if the \code{outreg} function call sets the argument \code{centering = \textquotedbl dcolumn\textquotedbl}
or \code{centering = \textquotedbl siunitx\textquotedbl}, then the
decimal-centering of columns will be attempted. NOTE: it is necessary
for the user to insert either \code{\textbackslash usepackage\{dcolumn\}}
...
In the past, my concern about the \code{dcolumn} package was that
it turned all of the text in columns into math mode italics, so many
tables either had unexpected style changes or failed to compile altogether.
It turned out that \code{siunitx} had the same impact, so the same
solutions were needed to correct the problem. (The solution is to
protect all text from that transformation, which is a primary source
of errors in this function at the current moment. Tell me what works!)

\section{Get the test version}
...
I've already noticed a couple of quirks while testing 1.8.125, so
by the time you try this, you'll get newer.

\section{User Interface question}

In this iteration of the effort, I've allowed the user to specify
one argument for column centering. The parameter ``\code{centering}''
has legal values c(``none'', ``dcolumn'', ``siunitx''). What
is your thought on that?

It is almost certain to me now that siunitx is the best approach,
by far, and I'm using it almost all the time in my projects.
...
\begin{table}
    ...
    \label{tab:ex1}% means that for cross referencing, call this float "tab:ex1"
\begin{tabular}
...
<<ex1w,results=tex>>=
library(rockchalk)
ex1w <- outreg(m1, title = "My Wide Format \"side-by-side\" columns (uncentered)",
...
<<ex1d, results=tex>>=
library(rockchalk)
ex2d <- outreg(m1, title = "Tight column with centering = \"dcolumn\"",
               label = "tab:ex2d", centering = "dcolumn",
               float = TRUE, print.results=FALSE)
cat(ex2d)
@
...
<<ex1s, results=tex>>=
library(rockchalk)
ex2s <- outreg(m1, title = "Tight column with centering = \"siunitx\"",
               label = "tab:ex2s", centering = "siunitx",
               float = TRUE)
@
...
ex1wd <- outreg(m1, title = "Wide (not tight) format with centering = 'dcolumn'",
                label = "tab:ex2wd", tight = FALSE, centering = "dcolumn",
                float = TRUE, print.results = FALSE)
cat(ex1wd)
@
...
ex1ws <- outreg(m1, title = "Wide (not tight) format with centering='siunitx'",
                label = "tab:ex2ws", tight = FALSE, centering = "siunitx",
                float = TRUE, print.results = FALSE)
cat(ex1ws)
@
...
ex2p <- outreg(list("Fingers" = m1), tight = FALSE, 
         title = "Ability to change p values (not centered)", 
         label = "tab:ex2p",  float = TRUE,
...
ex2pd <- outreg(list("Fingers" = m1), tight = FALSE, 
         ...
         label = "tab:ex2pd", centering = "dcolumn",
         float = TRUE,
         alpha = c(0.1, 0.05, 0.01)) 
@
...
ex2ps <- outreg(list("Fingers" = m1), tight = FALSE, 
         ...
         label = "tab:ex2ps", centering = "siunitx", float = TRUE,
         alpha = c(0.1, 0.05, 0.01)) 
@
...
ex3 <- outreg(list("Model A" = m1, "Model B has a longer heading" = m2),
         ...
         title = "My Two Linear Regressions (uncentered)", label = "tab:ex3",
...
ex3bd <- outreg(list("Model A" = m1, "Model B" = m2),
         ...
         label = "tab:ex3bd", centering = "dcolumn")
@
...
ex3bs <- outreg(list("Model A" = m1, "Model B" = m2),
         ...
         label = "tab:ex3bs", centering = "siunitx")
@
...
ex5dd <- outreg(list("Whichever" = m1, "Whatever" = m2),
         ...
         label = "tab:ex5dd", showAIC = TRUE, float = TRUE, centering = "dcolumn")
@

<<ex5ds, results=tex>>=
ex5ds <- outreg(list("Whichever" = m1, "Whatever" = m2),
         title = "Still have showAIC argument (siunitx)",
         label = "tab:ex5ds", showAIC = TRUE, float = TRUE, centering = "siunitx")
@
...
ex6d <- outreg(list("Whatever" = m1, "Whatever" =m2),
         ...
         runFuns = c("AIC" = "Akaike IC"), centering = "dcolumn", print.results=FALSE)
cat(ex6d)
@
...
<<results=tex>>=
ex7d <- outreg(list("Amod" = m1, "Bmod" = m2, "Gmod" = m3), centering = "dcolumn",
