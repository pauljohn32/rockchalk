...
This is the Fall, 2018 update of the rockchalk package. I offer a
course in regression analysis for social and behavioral scientists
...
\ref{tab:Combined-OLSGLM}. Table \ref{tab:Tab1} is the ``wide''
format output (separate columns for estimated parameters and standard
errors) for three models, obtained from \code{outreg(list(m1, m2, m3), centering=''siunitx'')}.
...
sized. The default output from \code{outreg} will have left-aligned
columns. Because users requested decimal-centered columns, the argument
centering has been introduced. If \code{centering=\textquotedbl siunitx\textquotedbl}
