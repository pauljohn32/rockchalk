...
\thanks{In case you want to be involved in development, the source code is
...
some limitations of R's termplot function. Version 1.8 introduces
a consistent framework for the creation of ``newdata'' objects and
the calculation of predicted values so that the marginal effects of
separate predictors can be easily explored.
\end{abstract}
% In document Latex options:
\fvset{listparameters={\setlength{\topsep}{0em}}}
...
and use the newdata function to create the examplar predictor data
...
and ask for more stars, as illustrated in Table \ref{tab:Tab2}. In
Table \ref{tab:Combined-OLSGLM}, observe that the linear and generalized
...
or conference paper. There are some warts in this output. Because
the model labels are not equal in length, the columns are not equally
sized. Because many \LaTeX{} distributions do not include the dcolumns
package, I've not aligned the coefficient estimates vertically as
some users might prefer. It seems to me that, if the vertical alignment
of estimates along the decimal is required, most users will install
dcolumn and make a few obvious changes in the markup that I provide.

\begin{table}
\protect\caption{Default Outreg Table\label{tab:Tab1}}
...
\begin{table}
\protect\caption{My Spread Out Regressions\label{tab:Tab2}}
...
\begin{table}
\protect\caption{Combined OLS and GLM Estimates\label{tab:Combined-OLSGLM}}
...
In rockchalk 1.8, a translator from \LaTeX{} markup to simple HTML
was introduced. The function outreg2HTML() can write an HTML code
file that a word processor program such as Libre Office or Microsoft
Word is able to import without a struggle. 
...
\protect\caption{plotSlopes: Linear Model with Confidence Interval\label{fig:ps05}}
\end{figure}
...
\protect\caption{plotSlopes Illustrated\label{fig:ps10}}
\end{figure}
...
\protect\caption{plotSlopes with a Categorical Moderator\label{fig:ps20}}
\end{figure}
...
\protect\caption{plotSlopes: the interval argument\label{fig:ps21}}
\end{figure}
...
in conducting some diagnostic analysis of the interactive terms. Aiken
and West (1991) (and later Cohen, Cohen, West, and Aiken) propose
...
\protect\caption{testSlopes for an Interactive Model\label{fig:ts10}}
\end{figure}
...
the calculation of the so-called Johnson-Neyman interval (1936), a
plot of which is presented in Figure \ref{fig:ts10}.

The method of calculation is outlined in Preacher, Curran, and Bauer
(2006). The values of $modx$ associated with a statistically significant
...
is rather similar to R's own termplot() function, but plotCurves()
has two major advantages. First, it allows interactions, and second,
it handles some complicated formulae that termplot() is not able to
manage. 
...
\protect\caption{plotCurves\label{fig:pcps20}}
\end{figure}


We fit a model with an elaborate specification and create the plot
in Figure \ref{fig:pcps20} with these commands:
...
\protect\caption{plotPlane for the Interactive Model m4\label{fig:pp100}}
\end{figure}
...
\protect\caption{Making plotSlopes and plotPlane Work Together\label{fig:pp110}}
\end{figure}
...
for a strong cautionary argument against them, see King 1986). R offers
no such thing as standardized regression, probably because this practice
is practice is thought to be mistaken. The automatic standardization
...
\begin{table}
\protect\caption{Comparing Ordinary and Standardized Regression\label{tab:stdreg10}}
...
it is often recommended as a way to obtain smaller standard errors
or to make estimates more numerically stable. 
...
from fluctuation. In R, when we run th \inputencoding{latin9}\lstinline!lm(y ~ x1 * x2)!\inputencoding{utf8},
...
centering as a way to deal with numerical instability are Aiken and
West (1991), who integrated that advice into the very widely used
regression textbook, \emph{Applied Multiple Regression/Correlation
for the Behavioral Sciences} (Cohen, et. al , 2002). They claim that
the inclusion of interactions causes ``inessential multicollinearity''
that is alleviated by centering. The advice is widely followed. One
statistics book intended for biologists observed, for example, ``We
support the recommendation of Aiken \& West (1991) and others that
multiple regression with interaction terms should be fitted to data
with centered predictor values'' (Quinn and Keough, 2002, Chapter
6). 

Technical rebuttals have been published already (Kromrey, J. D., \&
Foster-Johnson, L. , 1998; Echambadi and Hess, 2007), but the matter
still seems not widely understood. The argument is not that mean-centering
(or residual-centering) is wrong, but rather that it is unnecessary.
...
\begin{table}
\protect\caption{Comparing Regressions\label{tab:meancenter10-1}}
...
\protect\caption{plotSlopes for the centered and non-centered regressions\label{fig:pscentering}}
\end{figure}
...
\protect\caption{Predicted Planes from Centered and Uncentered Fits Identical\label{fig:mcenter30}}
\end{figure}
...
\protect\caption{Predicted Values of Mean and Residual-centered Models\label{fig:rcenter40}}
\end{figure}
...
normal generator.
\end{description}
Development on the package will continue on the basis of classroom
experience.
...
Design and Data Analysis for Biologists. Cambridge University Press.\end{thebibliography}
