...
My first approach to this was to teach a streamlined sequence of ``can't
miss'' steps. After that, I wrote functions newdata() and predictOMatic()
that will do most of this automatically. In order to understand what's
going on, I need the reader to understand the steps in principle. 

First, create a new data frame that sets the predictors at their central
values (mean or mode). Second, specify some alternate levels for particular
predictors in a new data frame. Third, combine those special interest
...
How to automate all of that, and make sure it works for most of the
regression formulae that are likely to come along? It's not good enough
to manage y \textasciitilde{} x1 + x2, we also need to handle y \textasciitilde{}
x1 + log(x2 + alpha) + ploy(x3, d) and so forth. That was a hard problem
for me, but I believed it is solved now (the secret recipe is a new
function called model.data()). User's won't need to worry about those
details. Instead, I suggest the use either newdata() with their own
predict() code, or a new ``all in one'' function I'm offering called
predictOMatic().

The function newdata() is a way of asking for a new data frame that
demonstrates certain focal values of one or more variables. First,
fit a regression, such as m1 above, which used the Chile data frame.
The newdata() function accepts an argument fl, which is short for
``focus list.'' That list is a set of values on which we want to
focus in the new data object. It is easier to understand with some
examples. The specification of the possible values can either be specific
values, as we see here:

<<echo=T>>=
mynewdf <- newdata(m1, fl = list(age = c(20, 30, 40), region = c("SA", "C","N")))
mynewdf
@\\
or it may incorporate calculated values that select focal points for
us:

<<echo=T>>=
mynewdf <- newdata(m1, fl = list(age = summChile$numerics[2:4, "age"], region = getFocal(Chile$region, n = 3)))
mynewdf
@\\
The getFocal() function is new in rockchalk. It is a generic function,
which means that it can receive variables of different types and it
is supposed to notice and ``do the right thing.'' In this case,
it notices that region is a factor, and it selects the 3 most frequently
observed scores. The newdata() output can be used in a predict() function
...
\begin{Sinput}
> mynewdf <- newdata(m1, fl = list(age = summChile$numerics[2:4, "age"], place = c("SA", "C","N")))
Error. The focus list: fl requests variables that are not included in the original model. 
...
The user can optionally supply a ``focus list'', the focal values
for which predictions are desired. However, if no fl argument is supplied,
then predictOMatic() will cycle through the variables in a fitted
model, calculating marginal predicted values for each of the variables. 

<<echo=T>>=
mypred1 <- predictOMatic(m1, fl = list(age = summChile$numerics[2:4, "age"], region = c("SA", "C","N")))
mypred1
mypred2 <- predictOMatic(m1)
mypred2
@

Under the hood, this required some hard work, a frustrating chain
of trial and error that is discussed in the vignette \emph{Rchaeology},
...
That output object is used by the function testSlopes(), which is
discussed below.
