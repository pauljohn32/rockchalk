...
\title{Using rockchalk for Regression Presentations (1.7.x development)}
...
\section*{Development on version 1.7.x}

The ``stable'' version of rockchalk on CRAN is 1.6.3. Version 1.7.x
is an ``unstable'' testing branch to add features. The next stable
release will be numbered 1.8.0. 

Test versions can be downloaded \& installed from our local test repository,
can be retrieved with:

\begin{Sinput}
install.packages("rockchalk", repos = "http://rweb.quant.ku.edu/kran", type = "source")
\end{Sinput}I don't upload versions unless they ``run'' and pass the usual tests
for uploading packages on CRAN, so I think they are mostly safe for
you to test. CRAN now asks we not upload packages frequently, so the
KRAN alternative is used for rapid-release development.

As always, the best way to learn what this does is to run the examples.

Installed with the package will be a directory called ``examples''.
I recently learned that it is possible to include not only the examples
that you run with the R function example(), but also other demonstration
files. These demonstration files are not linked to particular functions
in the package, they are ``just there'' waiting for you to explore.
For programming tutorial examples, look for ``noWords-{*}.R'' and
other filenames that do not end in ``{*}-ex.R''. 

The vignettes distributed with the package are
\begin{itemize}
\item rockchalk: a survey of package features, lags behind development slightly
(because I don't write out explanations until things actually work)
\item Rstyle: an essay about how my students should write R code
\item Rchaeology: an essay about difficult problems that I've had to solve
while working on rockchalk.
\end{itemize}
...
functions like mcGraph1(), mcGraph2(), mcGraph3(), and plotPlane()
come in handy. These don't do any work that is particularly original,
but they do help to easily make the multidimensional plots that turn
out ``about right'' most of the time. 
...
and this sometimes proves frustrating. The functions plotSlopes is
offered as my suggestion for creating interaction plots of ``simple
slopes''. The plotSlopes() function is intended for ``straight lines,''
but as soon a students discover log(x) or poly(x, 2), then they want
predicted lines plotted as well. That is why I created plotCurves().
These functions handle the work of calculating predicted values and
...
use the examples to build up more complicated drawings. Most recently,
a function to transfer the lines from plotSlopes() and plotCurves()
onto the plotPlane() drawings was added (look for the function addLines().
...
some folks still want to calculate ``beta weights.'' Hence the function
standardize(). 


A person might want to mean-center some predictors, or standardize
variables selectively the model. The function meanCenter() can optionally
center and/or standardize variables and re-run a regression. Mean
centering of variables, or its cousin ``residual centering,'' are
sometimes proposed enthusiastically as analytical simplifications
or statistical fixes for various perceived maladies. Because other
people propose those methods to the students, I have to provide functions
to implement them so we can investigate. I've concluded that those
methods are not truly helpful, and below I have an illustration. 
...
to use the returned value from summary() for further analysis. It
offers a large block of text in a table format. 
...
Two functions, newdata() and predictOMatic(), will greatly facilitate
the creation of ``newdata'' objects for use with R's predict methods. 

In order to understand the benefits of the approach proposed here,
it is necessary to understand the ``old way'' we were supposed to
do this in R. There are 3 steps in the usual procedure. First, fit
a regression, like so. This uses the Chile data set from the car package.

<<echo=T>>=
m1 <- lm(statusquo ~ age + income + population + region + sex, data=Chile)
@\\
Second, we need to build a newdata obect. It must be a new data frame
that includes values for all of the predictors. I usually do this
by extracting the data frame that was used to fit the model 

<<echo=T>>=
m1mf <- model.frame(m1)
@

\noindent and it will help us to make a new data object to be used
along with the predict function. 

\noindent Third, we need to use that new data object with the regression
model's predict method. If we were to run
...
That doesn't simplify anything. Instead, we want predicted values
for a focal set of values, values that are substantively interesting
(say, quantiles or other variations above and below the mean). I'll
explain details on how to create this new data object below, lets
just suppose it exists. R's predict function has a newdata argument
that will offer predictions for those focal cases, if only we could
construct myNewDF.
...
were used in the regression formula. I've taught R for almost a decade
and this is often a source of frustration. 

I still teach the Three Step sequence of ``can't miss'' steps. But
I expect I won't be doing that for long because the functions newdata()
and predictOMatic() will do most of this automatically. In order to
understand what's going on, I need the reader to understand the steps
in principle. 

Now, what kind of a process is required to create a workable newdata
frame? First, calculate values for the predictors at their central
values (say, mean or mode or median). The summarize function and the
centralValues functions are run first to collect up the needed information.
...
Second, specify some alternate levels for particular predictors in
a new data frame. 

<<>>=
focalVals <- list(age = m1mfsumm$numerics[2:4, "age"], region = c("C","N"))
@

R's expand.grid() function offers a very convenient way to get all
combinations of those predictors.

<<echo=T>>=
mixAndMatch <- expand.grid(focalVals)
mixAndMatch
@

Third, combine those focal values with the central values for the
other variables in the data set. Recall that we must specify values
for all of the predictors in the model. There is a danger that we
may end up with columns that have duplicate names (there is a variable
``age'' in both m1cv and mixAndMatch). To avoid duplicate column
names, I keep only the columns from m1cv that are not already in mixAndMatch
...
the data management is slightly more tedious. I usually prefer to
rename the fit and lower and upper confidence interval regions so
that they have unique names.

<<echo=T>>=
mynewdf <- cbind(mixAndMatch, m1cv[  ,!colnames(m1cv) %in% colnames(mixAndMatch)])
preds <- predict(m1, newdata = mynewdf, interval="confidence")
colnames(preds) <- paste("m1", colnames(preds), sep = "")
mynewdf <- cbind(mynewdf, preds)
mynewdf
@

That seems like a lot of work to me, and I've done it many times. 

Can this be made easier? And can it be robust and flexible? I believe
the answer is yes. The newdata() and predictOMatic() functions handle
the details and allow the user to adjust their requests to allow for
very fine grained control. Here are the key arguments for these functions.
\begin{enumerate}
...
select divider algorithms. It is also allowed to declare particular
values.
\end{enumerate}
The function newdata() is a way of asking for a new data frame that
demonstrates certain focal values of one or more variables. We start
with a fitted regression, m1, which was fitted above. This asks for
a ``mix and match'' of values from age and region. 
...
mynewdf
@\\
The function getFocal() is a generic function, which means that it
can receive variables of different types and it is supposed to notice
and ``do the right thing.'' It allows the section of algorithms
as explained in the documentation, but the default for numeric variables
is the quantile and for factors the most frequenly observed outcomes
are selected. The newdata() output can be used in a predict() function
call as demonstrated above. 

Note that I've tried to help users to catch typographical errors in
the predVals. 

\begin{Sinput}
> mynewdf <- newdata(m1, predVals = list(age = summChile$numerics[2:4, "age"], place = c("SA", "C","N")))
Error. The focus list: predVals requests variables that are not included in the original model. 
The names of the variables in the focus list be drawn from this list: age income population region sex
\end{Sinput}

I've tried to go one step further, offering a function called predictOMatic().
predictOMatic() receives a regression model and if no other details
are supplied, it takes each variable, one at a time, and collects
some focal values and estimates predictions for each one. The user
can request that particular values of the predictors are used, or
can declare one of several algorithms for selection of focal values.
All of these details are managed by the argument called predVals,
which is described in the help pages (with plenty of examples). 

<<echo=T>>=
mypred1 <- predictOMatic(m1)
mypred1
...
with complicated regression formulae. It is fairly easy to write a
prediction scheme for a simple linear equation like y \textasciitilde{}
x1 + x2, we also need to handle y \textasciitilde{} x1 + log(x2 +
alpha) + ploy(x3, d) and so forth. That was a hard problem for me,
but I believed it is solved now (the secret recipe is a new function
called model.data()). Under the hood, this required some hard work,
a frustrating chain of trial and error that is discussed in the vignette
\emph{Rchaeology}, which is distributed with this package. 
...
outreg(m1, tight=FALSE, modelLabels=c("Fingers"), alpha = c(0.05, 0.01, 0.001))
@
\end{table}
...
outreg(list(m1,m2), modelLabels=c("Mine","Yours"), varLabels = list(x1="Billie"), alpha = c(0.05, 0.01, 0.001))
@
...
outreg(list(m1,m2), tight=FALSE,  modelLabels=c("Mine","Yours"), varLabels = list(x1="Billie"), alpha = c(0.05, 0.01, 0.001))
@
\end{table}
...
<<results=tex, echo=F>>=
outreg(list(m1,m2,m3), tight=F, modelLabels=c("I Love love love really long titles","Hate Long","Medium"), alpha = c(0.05, 0.01))
@
\end{table}
...
A new feature in the 1.7.x series is the ``interval'' argument,
which will draw shaded confidence or predicted value intervals. I
don't calculate these intervals. These intervals are calculated by
R's predict.lm() function. These intervals use ``transparent'' shading
to help us see overlaps. In order to make this more useful, we limit
the number of values of the moderator that are investigated. See Figure
\ref{fig:ps21}.

\begin{figure}
<<ps21, fig=T, echo=F>>=
m5psa <- plotSlopes(m5, plotx = "x1", modx = "x4", modxVals = c("West","East"), xlab = "x1 is a Continuous Predictor", xlim=magRange(dat$x1, c(1.2,1)), interval = "conf")
@

\caption{plotSlopes: the new interval option\label{fig:ps21}}
\end{figure}


This would work for any kind of regression model that supplies a predict
function that answers to the interval argument. Unfortunately, predict.glm()
does not do so. 

Quite often, students want to display predicted values with pleasant
confidence intervals. I've re-worked plotSlopes() so that it will
make a pleasant plot for a single value of a moderator, as shown in
Figure \ref{fig:ps22}

\begin{figure}
<<ps22, fig=T, echo=F>>=
m5psa <- plotSlopes(m5, plotx = "x1", modx = "x4", n = 1,  xlab = "x1 is a Continuous Predictor", xlim=magRange(dat$x1, c(1.2,1)), interval = "conf")
@

\caption{plotSlopes: the new interval option\label{fig:ps22}}
\end{figure}
