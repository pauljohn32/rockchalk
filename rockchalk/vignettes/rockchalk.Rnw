...
or <- outreg(list(m1, m2, m3), siunitx=TRUE)
@
\end{table}
...
or1 <- outreg(list("The First Model with a Long Title" = m1, "Another Model" = m3), tight = FALSE, alpha = c(0.05, 0.01, 0.001), siunitx=TRUE)
@
\end{table}
...
or2 <- outreg(list(m1,gm1), modelLabels = c("OLS:y","GLM: Categorized y"), siunitx=TRUE)
@
\end{table}
...
or10 <- outreg(list(m4, m4s), tight = F, modelLabels = c("Not Standardized","Standardized"), siunitx=TRUE)
@
\end{table}
...
or11 <- outreg(list(m6linear, m6int, m6mc, m6rc), tight=F, modelLabels=c("Linear", "Interaction","Mean-centered","Residual-centered"), alpha = c(0.05, 0.01), siunitx=TRUE)
@
\end{table}
