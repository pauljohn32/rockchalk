...
\end{lstlisting}
\inputencoding{utf8}\item [{stop()}] Halts a function and returns a message to the user. Usage
example 

\inputencoding{latin9}\begin{lstlisting}
if (isTRUE(x)) stop(paste("I'm sorry, you failed to supply the right information.")))
\end{lstlisting}
\inputencoding{utf8}\item [{stopifnot()}] It is so frequent to run ``\code{if(isTRUE(x)) stop()}''
...
\end{lstlisting}
\inputencoding{utf8}\item [{deparse(substitute())}] Inside a function, it is often important
