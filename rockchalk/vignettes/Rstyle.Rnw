...
with them.
\item [{Rchaeologist:}] One who practices Rchaeology.\end{description}
%
...
\footnote{Yet :)%
} With one exception at the end of this note, none of the advice here
is ``my'' advice. Instead, it is my best description of the standards
followed by the leading R programmers. 
...
} In what follows, I'll try to explain why there are some variations
among these projects and offer some advice about how we (the users)
should sort through their advice.
...
\begin{quote}
Open the source code for R. I mean, literally, download R-2.15.3.tar.gz
...
\footnote{\url{http://www.gnu.org/prep/standards/standard.html}%
...
\item After reviewing the R source code, I am uncertain about whether one
ought to insert one space after the function names ``if'' and ``for''.
In a language like C, ``if'' and ``for'' are keywords, and there
would be a space, as in ``\inputencoding{latin9}\lstinline!if (x < 7)!\inputencoding{utf8}''.
However, in R, \inputencoding{latin9}\lstinline!if()!\inputencoding{utf8}
and \inputencoding{latin9}\lstinline!for()!\inputencoding{utf8} are
functions, and we do not insert spaces between function names and
their parentheses. About 90\% of the time, the R source does not include
a space after ``if'' or ``for''. 
\end{enumerate}
...
within a namespace, but doing so requires a bit of effort and mischief.%
\footnote{In case you wonder, here's how to cause the worst case scenario.
\begin{lyxcode}
nseq~<-~function(x)~print(\textquotedbl{}Hello,~good~to~see~you\textquotedbl{})

assignInNamespace(\textquotedbl{}seq.default\textquotedbl{},~nseq,~\textquotedbl{}base\textquotedbl{})\end{lyxcode}
%
}
...
\footnote{\url{http://www.r-project.org/mail.html}%
} or on web forums %
\footnote{e.g., \url{http://stackoverflow.com/questions/tagged/r}%
